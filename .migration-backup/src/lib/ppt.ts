...
export async function generatePPTX(
  ...
  slides.forEach((slide, index) => {
    ...
    if (slide.type === "title" || index === 0) {
      ...
      s.addText(slide.title || projectTitle, {
        x: 0.5,
        y: 1.8,
        w: "90%",
        h: 1.5,
      ...
      const subtitle = Array.isArray(slide.content)
        ? slide.content.join("\n")
      ...
      s.addText("Powered by Hackathon Helper AI", {
        x: 0.5,
        y: 4.7,
        w: "90%",
        h: 0.5,
    ...
    if (slide.type === "screenshots") {
      ...
      const note = Array.isArray(slide.content)
        ? slide.content.join("\n")
        : slide.content || "";
      if (note) {
        s.addText(note, {
    ...
    s.addText(content, {
      ...
      valign: "top",
      fontFace: "Arial",
      bullet: Array.isArray(slide.content),
    });
  });

  await pres.writeFile({
    fileName: `${slugify(projectTitle)}_Presentation.pptx`,
  });
}

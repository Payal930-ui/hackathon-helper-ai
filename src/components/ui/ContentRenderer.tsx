...
import { CheckCircle2, AlertTriangle, Lightbulb, Copy, Calendar, User, Clock, ShieldCheck, Target, TrendingUp } from "lucide-react";
import { Button } from "./Button";
import toast from "react-hot-toast";
import type { GeneratedResults, OutputKey } from "@/lib/types";
          ...
          {Object.entries(data).map(([key, value]: [string, any]) => (
            key !== "reasoning" && (
              <div key={key} className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-3xl border border-gray-100 dark:border-gray-800">
                ...
                <h4 className="text-xl font-bold mt-1 dark:text-white">{value}</h4>
              </div>
            )
          ))}
          {data.reasoning && (
